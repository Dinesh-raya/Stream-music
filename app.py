    ...
    # Apply a minimal dark theme when selected
    if theme == "Dark":
        st.markdown(
            """
            <style>
    ...
    st.title("🎧 Streamlit Music Player (Final)")
    st.caption("Audio-only playback with HLS (.m3u8) support. No downloads.")

    def play_audio(audio_url: str):
        """
        Play audio_url inline. If it's an HLS (.m3u8) stream, inject hls.js-based player.
        For direct audio files, use Streamlit's st.audio.
        """
        ...
        if audio_url.lower().endswith(".m3u8"):
            # Use a safe HTML snippet and inject the audio URL using % formatting to avoid
            # any accidental Python f-string brace collisions.
            player_html = (
            ...
            # For non-HLS audio, Streamlit's st.audio is the simplest approach.
            try:
                st.audio(audio_url)
    ...
    def handle_play(song):
        # Fetch audio URL from API and play
        try:
            ...
            st.error(f"Unexpected error while fetching audio: {e}")
            ...
            with st.expander(f"{idx}. {song.get('title', 'Untitled')}"):
                ...
                with c3:
                    if st.button("Remove", key=f"queue_remove_{song.get('id')}"):
                        st.session_state.playlist = [s for s in st.session_state.playlist if s.get("id") != song.get("id")]
